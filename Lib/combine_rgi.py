...
def combine_rgi(sample_files, outdir, columns):
    # Create an empty combined table
    combined_table = pd.DataFrame()

    # Create an empty list to store intermediate tables
    intermediate_tables = []
    ...
    for column in columns:
        ...
        # Append the new DataFrame to the intermediate tables list
        intermediate_tables.append(combined_data)
    ...
    # Merge all intermediate tables on the common columns
    combined_table = intermediate_tables[0] # Initialize with the first table
    for i in range(1, len(intermediate_tables)):
        combined_table = pd.merge(combined_table, intermediate_tables[i], on=common_columns, how='outer')

    # Save the final combined table to a single output file
    combined_table_file = os.path.join(outdir, 'RGI_combined_table.csv')
    combined_table.to_csv(combined_table_file, index=False, header=True)
